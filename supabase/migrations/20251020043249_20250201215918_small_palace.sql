...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'services' AND policyname = 'Anyone can read active services'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'appointments' AND policyname = 'Only authenticated users can manage appointments'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'booking_rules' AND policyname = 'Anyone can read booking rules'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'booking_rules' AND policyname = 'Allow anon to insert default booking rules'
  ) THEN
